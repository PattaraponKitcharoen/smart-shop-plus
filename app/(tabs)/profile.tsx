...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    const prepare = async () => {
      try {
        ...
        console.error("Profile DB Init Error:", e);
      }
    };
    prepare();
  }, []);
  ...
  const handleSaveName = () => {
    if (tempName.trim() === '') {
      if (Platform.OS === 'web') alert("กรุณาระบุชื่อที่ต้องการ");
      else Alert.alert("ผิดพลาด", "กรุณาระบุชื่อที่ต้องการ");
      return;
    }
    setName(tempName);
    setIsEditing(false);
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      if (allItems.length === 0) {
        if (Platform.OS === 'web') alert("คุณยังไม่มีรายการสินค้าที่จะส่งออก");
        else Alert.alert("ไม่พบข้อมูล", "คุณยังไม่มีรายการสินค้าที่จะส่งออก");
        return;
      }
      ...
      console.error(error);
      if (Platform.OS === 'web') alert("ไม่สามารถส่งออกข้อมูลได้");
      else Alert.alert("เกิดข้อผิดพลาด", "ไม่สามารถส่งออกข้อมูลได้");
    }
  };
  ...
  const handleImportCSV = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        content = await FileSystem.readAsStringAsync(fileUri, { encoding: "utf8" });
      }
      ...
      await fetchData();
      if (Platform.OS === 'web') alert(`นำเข้าข้อมูลเรียบร้อยแล้ว ${importedCount} รายการ`);
      else Alert.alert("สำเร็จ", `นำเข้าข้อมูลเรียบร้อยแล้ว ${importedCount} รายการ`);
    } catch (error) {
      console.error(error);
      if (Platform.OS === 'web') alert("ไม่สามารถนำเข้าข้อมูลได้");
      else Alert.alert("ผิดพลาด", "ไม่สามารถนำเข้าข้อมูลได้");
    }
  };

  const handleClearAllData = async () => {
    const msg = 'คุณแน่ใจหรือไม่ว่าต้องการล้างฐานข้อมูลแอปทั้งหมด? ข้อมูลนี้ไม่สามารถกู้คืนได้';
    ...
    const performDelete = async () => {
      try {
        ...
        await fetchData();
        if (Platform.OS === 'web') alert("ล้างข้อมูลเรียบร้อยแล้ว");
        else Alert.alert("สำเร็จ", "ล้างข้อมูลเรียบร้อยแล้ว");
      } catch (error) {
        console.error(error);
        if (Platform.OS === 'web') alert("ไม่สามารถล้างข้อมูลได้");
        else Alert.alert("ผิดพลาด", "ไม่สามารถล้างข้อมูลได้");
      }
    };
    ...
    if (Platform.OS === 'web') {
      ...
      Alert.alert('ลบข้อมูลทั้งหมด', msg, [
        { text: 'ยกเลิก', style: 'cancel' },
        { text: 'ลบข้อมูล', style: 'destructive', onPress: performDelete }
      ]);
    }
  };
